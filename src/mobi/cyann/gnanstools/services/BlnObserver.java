 ...
 */
package mobi.cyann.gnanstools.services;
...
public class BlnObserver extends FileObserver {
	private final static String LOG_TAG = "GNaNSTools.BlnObserver";
	...
	public BlnObserver(Context context) {
		...
		context.registerReceiver(alarmReceiver, new IntentFilter("mobi.cyann.gnanstools.SHUTDOWN_BLN"));
	}
		...
		if(event == FileObserver.CLOSE_WRITE) {
			...
			if(blnTimeout > 0) {
				...
				if(n > 0 && sc.getLastResult(0).equals("1")) { // BLN on
					...
					PendingIntent pendingIntent = PendingIntent.getBroadcast(context, 1, new Intent("mobi.cyann.gnanstools.SHUTDOWN_BLN"), PendingIntent.FLAG_CANCEL_CURRENT);
